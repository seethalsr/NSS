...
class NssPrinci extends CI_Controller 
{
	...
	public function create_po()
	{
		...
		$queryprinci['data'] = array(
						...
						'po_join_date'=>$f_date,
						'po_uploaded_img'=>($_FILES['txt4']['name']),
						'college_id'=>$queryprinci['college_id'],
						'po_status'=>'active',
		...
		//photoupload section
		 $path   = 'upload/princi/col'.$queryprinci['college_id'].'/poimage/';
		 if (!is_dir($path)) 
		 { //create the folder if it's not  exists
			mkdir($path, 0777, TRUE);
		 }
		 $name = preg_replace('#[^\pL\pN/-]+#', '',  $queryprinci['data']['po_email']); 
		$config = array(
                    'allowed_types' => 'jpg',
					'overwrite'     => TRUE,
                    'upload_path' => $path,
		...
		$done = $this->upload->do_upload('txt4');
		if($done)
