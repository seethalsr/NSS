...
class NssPo extends CI_Controller 
{
	...
	{	
		...
		if($this->input->post('upd'))
		{
		  $upd_data = array(
		  'nss_stud_id' => $id,
		  'account_student_email'=> $this->input->post('txt1'),
		  'account_student_mobileno'=> $this->input->post('txt2'),
		  'blood_group'=> $this->input->post('blood_group'),
		  'donate'=> $this->input->post('donate'),
		  'donate_month' => $this->input->post('month'),
		  'donate_yr'=>$this->input->post('donate_yr'),
		  );
	...
	public function nss_certi()
	{
		...
		{		
			...
			{
				$querypo['eli_dat'] = $this->Publicmodel->elig_rep($querypo['college_id'],$querypo['batch_period'],$querypo['unit_list']);
			}
		}
		$querypo['adminspan1'] = $this->load->view('common_sidebar',$querypo,true);
		$querypo['adminspan2'] = $this->load->view('common_template_topbar','',true);	
	...
	{	
		...
		$data['student_details'] = $this->Publicmodel->get_stud_certi($id);
		$this->load->library('Pdf');
		...
		$pdf->SetFont("dejavusans", "", 10);
			...
			// QRCODE,M : QR-CODE Medium error correction
		$pdf->write2DBarcode(base_url().'VerifyCertificate/'.$en.'/'.$code, 'QRCODE,M', 250, 170, 18, 18, $style, 'N');
