...
class NssPrinci extends CI_Controller 
{
	...
	public function nss_certi()
	{
		...
		$queryprinci['batch_period']=  $this->session->userdata('batch_period');	
			$issu='N';	
			$issu=$this->Publicmodel->get_certi_iss($queryprinci['college_id']);
			//print_r( $issu);exit;
			if($issu['fwd_coll']=='Y')
			{
				$queryprinci['eli_dat'] = $this->Publicmodel->get_elig_rep_princi($queryprinci['college_id'],$queryprinci['batch_period']);
			}
			else
			{
				$queryprinci['eli_dat'] = 'N';
			}
		...
		$queryprinci['adminspan1'] = $this->load->view('common_sidebar',$queryprinci,true);
		$queryprinci['adminspan2'] = $this->load->view('common_template_topbar',$queryprinci,true);	
		$queryprinci['listspan'] = $this->load->view('princi/nss_certi',$queryprinci,true);	
	...
	public function create_unit()
	{
		...
		$this->form_validation->set_error_delimiters('<span class="error_form_validation">', '</span>');
		$this->form_validation->set_rules('unit', 'Unit ID', 'required|callback_is_unique_unit['.$queryprinci['college_id'].']',array('required' => 'Please provide a valid %s.', 'is_unique_unit'=>'This %s is already registered   unit id',));
		...
		$unit_data = array(
								...
								'batch_period' => $this->input->post('batch'),
								'nss_unit_id'=> $this->input->post('unit'),
	...
	public function view_enroll_list()
	{
			...
			if($this->input->post('enroll'))
		{
			...
			{  if($queryprinci['college_id']<10) $college_id = '00'.$queryprinci['college_id']; 
				...
				'nss_stud_id' => $val,
				'nss_status' => 'active',
				'nss_enroll_id'=>preg_replace("/[^A-Za-z0-9]/", "", 'KL052019'.$val),// format KL05-UNIT NO-YEAR-sl.no eg:KL050011801(remove '-')
				); $j ++; 
			}
...
$result = $this->email->send();
	...
	function is_unique_unit($unitid,$col)
	{
		//echo $unitid;echo $batch;echo '<br></br>';echo $col;exit;
		
		$exist_id = $this->Publicmodel->check_unit_id($unitid,$col,'2017-2019');
		if($exist_id)
		return false;
		else
		 return true;
