...
class NssAdmin extends CI_Controller 
{
	...
	{
		$queryadmin['user_type'] = $this->session->userdata('user_type');
		$queryadmin['college_id'] = $this->session->userdata('college_id');
		$queryadmin['name']= $this->session->userdata('name');
		$queryadmin['college_name'] = 'KANNUR UNIVERSITY';
		$login_id = $this->session->userdata('login_id');
	...
	public function college_list()
	{
		...
		if($this->input->post('delete'))
		{
			$data_college_delete=array(
				...
				$college_delete_attempt=$this->load->Publicmodel->delete_clg($data_college_delete);
				//echo var_dump($data_college_delete);die;

		}
		//var_dump($queryadmin);die;
		//$college_id = $this->session->userdata('college_id');
		//var_dump($queryadmin);die;
		//$college_id=$this->input->post('delete');
		//$this->load->Publicmodel->delete_clg($college_id);
		//var_dump($queryadmin);die;
	...
	{/*
		...
		$this->load->view('common_template',$queryadmin);
	*/}
	public function admin_college_add()
	{
		$queryadmin['user_type'] = $this->session->userdata('user_type');
		$queryadmin['name']= $this->session->userdata('name');
		$queryadmin['college_name'] = 'KANNUR UNIVERSITY';
		...
		//var_dump($queryadmin['msg']);die;
		
		$this->form_validation->set_error_delimiters('<span class="error_form_validation">', '</span>');		
		$this->form_validation->set_rules('txt1');
		$this->form_validation->set_rules('txt2');
		$this->form_validation->set_rules('txt3');
		$this->form_validation->set_rules('txt4');
		//var_dump($this->form_validation->set_rules('txt4'));die;
		...
		$this->form_validation->set_rules('txt5');
		...
		if($this->input->post('submit5'))
		{
			$data = array(
						...
						'Email'=>$this->input->post('txt4'),						
						'Address'=>$this->input->post('txt2'),
						'District'=>$this->input->post('txt5'),						
						
						'Contact Number'=>$this->input->post('txt3'),
									
					);
		
		$ins=$this->Publicmodel->insert_new_college($data);
		...
		$queryadmin['adminspan3'] = $this->load->view('admin/admin_college_add','',true);	
		$this->load->view('common_template',$queryadmin);
	...
	//	else $queryadmin['msg']='';
		...
		//$this->Publicmodel->delete_clg($data);
	...
	public function po_list()
	{
		...
		if($type)
		{ 
			$queryadmin['college_name_sel'] = $this->Publicmodel->getlistcollege_dist($type);
			$college_name_sel = $this->input->post('name');
		if($college_name_sel)
		{
	...
	public function admin_manage()
	{
		...
		//var_dump($queryadmin['get_data_manage']);die;
	...
	public function admin_v_enroll_list()
	{
		...
		if($college_name_sel)
		{
			...
			elseif($queryadmin['sel_unit']){
				...
				//var_dump($queryadmin['enroll_list']);die;
	...
	public function eligibile_report()
	{
		...
		if($college_name_sel)
		{
			...
			if(empty($queryadmin['unit_list'])){
				...
				$queryadmin['msg']= '<h4 style="color:#FF0000;font-weight:bold;">UNIT IS NOT CREATED FOR THE THIS COLLEGE</h4>';}
			elseif($queryadmin['sel_unit']){
			$queryadmin['eli_det'] = $this->Publicmodel->get_elig_rep($college_name_sel,$queryadmin['sel_batch'],$queryadmin['sel_unit']);
				}
	...
	public function eligibile_report_old()
	{
		$queryadmin['msg']="";
		$queryadmin['user_type'] = $this->session->userdata('user_type');
		$queryadmin['name']= $this->session->userdata('name');
