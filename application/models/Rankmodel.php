...
class Rankmodel extends CI_Model {
	...
	public function get_aloted_seat($coll,$cou,$res)
	{
		...
		$this->db->where_in('status',array('alloted','admitted'));
	...
	public function update_rank_prev($college,$course,$rank,$res)
	{
		...
		$sql="UPDATE `ranking_rank_master` SET rank=rank-1 WHERE `college_id` = ".$college." AND `course_id` = ".$course." AND `reservation_id` = ".$res." AND `rank` > ".$rank." AND `status` in( 'alloted' , 'admitted ')";
