...
update nss_stud set elig='Y' where tot_hr>=240 and (splcamp='YES' OR mini1 ='YES' or mini2 ='YES')
 and ((enrolled_date like '2017%'and  enroll_end like '2019%')or (enrolled_date like '2016%'and  enroll_end like '2018%') );
 
 also 
(SELECT * FROM nss_stud where tot_hr>=240 and (splcamp='YES' OR mini1 ='YES' or mini2 ='YES')
 and ((enrolled_date like '2017%'and  enroll_end like '2019%')or (enrolled_date like '2016%'and  enroll_end like '2018%') ))
...
select a.nss_stud_id,a.nss_enroll_id,a.account_id,a.account_student_name,b.college_name_for_gradecard,a.specialisation_id,a.batch_period,
 a.nss_enroll_unit,'Prof. M. J. Mathew','2019-03-01 00:00:00','NSS PROGRAMME CO-ORDINATOR','active' from nss_stud as a
inner join nss_college as b
on a.college_id = b.college_id where a.elig='Y' 
;
...
class Publicmodel extends CI_Model {
	...
	public function issued($unit)
	{
		$query = $this->db->query('INSERT INTO cert_verify_2017_2019 (nss_stud_id, enroll_no, prn, name, college_name, course, nss_batch, nss_unit, verified_by, verified_on, verified_desig, status)
select a.nss_stud_id,a.nss_enroll_id,a.account_id,a.account_student_name,b.college_name_for_gradecard,a.specialisation_id,a.batch_period,
 a.nss_enroll_unit,"Prof. M. J. Mathew","'.date('Y-m-d H:i:s').'","NSS PROGRAMME CO-ORDINATOR","active" from nss_stud as a
inner join nss_college as b
on a.college_id = b.college_id where nss_unit_id='.$unit.';
');
 //echo $this->db->last_query();exit;
	...
	public function get_verified_elig_list()
	{
		$this->db->select('a.college_id,a.elig_chk,a.nss_enroll_unit,a.nss_unit_id,b.college_name,group_concat(a.nss_stud_id) as nss_stud_id,a.fwd_coll');
        $this->db->from('nss_stud as a');
		$this->db->join('nss_college b', 'a.college_id=b.college_id',  'inner');
		//$this->db->where('elig_chk','Y');
		$this->db->group_by('nss_unit_id');
		$this->db->order_by('a.elig_chk asc,a.fwd_coll asc,b.college_name asc');
		$query = $this->db->get();// echo $this->db->last_query();exit;
    	return $query->result_array();
		
	}
	public function update_isse_certi($unit)
	{
		$this->db->where('nss_unit_id',$unit); 
		$this->db->set('fwd_coll','Y');
		$this->db->update('nss_stud');
		return 1;
	}
	public function get_certi_iss($college_id)
	{
		$this->db->select('fwd_coll');
	...
	public function update_eligcheck($chk_data,$coll,$unit,$batch)
	{
		//print_r($chk_data);echo $coll.'//'.$unit.'//'.$batch.'//';exit;
		$this->db->where_in('nss_stud_id',$chk_data); 
		$this->db->set('elig_chk','Y');
		$this->db->update('nss_stud');
		
		$this->db->where_not_in('nss_stud_id',$chk_data);
		$this->db->where('college_id',$coll);	
		$this->db->where('batch_period',$batch);
		$this->db->where('nss_enroll_unit',$unit);
		$this->db->set('elig_chk','N');
		$this->db->update('nss_stud'); //echo $this->db->last_query();exit;
		
		
        return $this->db->affected_rows();
	}
	public function update_elig_for_sel($field,$val,$studs,$coll,$unit)
	{
	//	print_r( $field.'//'.$val.'//'.$studs.'//'.$coll.'//'.$unit);exit;
		switch($field)
		{
		case($field=='res'):
		$column='cast';break;
		case($field=='adm'):
		$column='admission_year';break;
		case($field=='gen'):
		$column='gender';break;
		case($field=='cou'):
		$column='specialisation_id';break;
		case($field=='estart'):
		$column='enrolled_date';break;
		case($field=='eend'):
		$column='enroll_end';break;
		case($field=='mini1'):
		$column='mini1';break;
		case($field=='mini2'):
		$column='mini2';break;
		case($field=='spl'):
		$column='splcamp';break;
		case($field=='splstart'):
		$column='splcamp_start';break;
		case($field=='splend'):
		$column='splcamp_end';break;
		case($field=='desti'):
		$column='spl_desti';break;
		case($field=='tot'):
		$column='tot_hr';break;
		...
		$this->db->where_in('nss_stud_id',$studs); 
		$this->db->where('college_id',$coll);
		$this->db->where('nss_enroll_unit',$unit);
	...
	public function get_elig_finnnn($coll,$unit,$batch)
	{
		 $this->db->select('*');
        $this->db->from('nss_stud');
		$this->db->where('college_id',$coll);	
	...
	public function get_elig($coll,$unit,$batch)
	{
		 $this->db->select('*');
        $this->db->from('nss_stud');
		$this->db->where('college_id',$coll);	
	...
	public function update_verify_po($coll,$unit,$batch)
	{
		$this->db->where('college_id',$coll);
		$this->db->where('nss_enroll_unit',$unit);
		$this->db->where('batch_period',$batch);	
	...
	{
		$this->db->select('STUD.blood_group,STUD.account_student_name,STUD.donate,count(STUD.nss_stud_id) as cc ,STUD.nss_stud_id,STUD.nss_enroll_unit,STUD.college_id,PO.po_name,PO.po_contact');
		$this->db->where('STUD.blood_group', $blood  );
		$this->db->where('STUD.donate', 'YES'  );
	...
	public function get_po($colge_id)
	{
		...
		$this->db->where('COL.college_id <> 213');
		...
		$this->db->order_by('COL.college_district', 'asc');
		$this->db->order_by('COL.college_name', 'asc');
		$query = $this->db->get();//echo $this->db->last_query();exit;
     	return $query->result_array();
	}
	...
	public function delete_clg($data_college_delete)
	{
		$college_delete_attempt = $this->db->delete('nss_college',$data_college_delete);
		//return $this_>db->delete_id();
		//$query = $this->db->delete('nss_college');
...
// echo $this->db->last_query();exit;
	...
	public function get_college_name1($type)
	{
		...
		$query = $this->db->get('nss_college');	//echo $this->db->last_query();exit;	
		return $query->result_array();
	}
...
// make this into singe function start*********************************************//	
	...
	public function update_batch_eligedit($data)
	{
		  $this->db->update_batch('nss_stud',$data,'nss_stud_id'); 
		  return 1;
	...
	public function prepare_elig_rep($coll,$batch,$unit)
	{
	...
	public function prepareelig($col,$batch,$unit)
	{
		
		$data1=array( 'elig'=>'Y' );
	 	$this->db->where('tot_hr >= 240');
		$this->db->where('(splcamp = "YES" or mini1 ="YES" or mini2="YES")'  );
		//$this->db->where('(splcamp_end <> "1970-01-01" )'  );
		//$this->db->where('(splcamp_start <> "1970-01-01" )'  );
		//$this->db->where('(splcamp_desti <> "" )'  );
		$this->db->where('year(enroll_end)-year(enrolled_date)=2');
		$this->db->where('year(enrolled_date) in ("2017","2016")');
		$this->db->where('college_id',$col);
		$this->db->where('batch_period',$batch);
		$this->db->where('nss_enroll_unit',$unit);
	 	$this->db->update('nss_stud',$data1);	//echo $this->db->last_query();exit;
		
		$data1=array( 'elig'=>'N' );
	 	$this->db->where('tot_hr < 240');
		$this->db->where('(splcamp <> "YES" and mini1 <>"YES" and mini2<>"YES")'  );
		$this->db->where('year(enroll_end)-year(enrolled_date)<>2');
		//$this->db->where_not('year(enrolled_date) in ("2017","2016")');
		$this->db->where('college_id',$col);
		$this->db->where('batch_period',$batch);
		$this->db->where('nss_enroll_unit',$unit);
	 	$this->db->update('nss_stud',$data1);//echo $this->db->last_query();exit;
	...
	{
		$this->db->select('nss_stud_id,account_id,account_student_name,specialisation_id,nss_enroll_unit,nss_enroll_id,batch_period,mini1,mini2,splcamp,tot_hr,elig,elig_chk')
          	->from('nss_stud');
			$this->db->where('college_id',$col);
			$this->db->where('batch_period',$batch);
			if($unit)
	...
	public function get_elig_rep_princi($col,$batch)
	{
		//echo $col.$batch;exit;
		$this->db->select('nss_stud_id,account_id,account_student_name,specialisation_id,nss_enroll_unit,nss_enroll_id,batch_period,mini1,mini2,splcamp,tot_hr,elig,elig_chk')
          	->from('nss_stud');
	...
	public function get_stud_unit($unit)
	{
		$this->db->select('group_concat(nss_stud_id) as nss_stud_id ')
          	->from('nss_stud');
			$this->db->where('nss_unit_id',$unit);
			$query = $this->db->get();//echo $this->db->last_query();exit; 
            return $query->row_array();
	}
	public function get_elig_rep_certi_studs($studs)
	{//print_r($studs['nss_stud_id']);exit;
		$arr=explode(",",$studs['nss_stud_id']);
		$this->db->select(' group_concat(nss_stud_id) as id');
          	$this->db->from('nss_stud');
			$this->db->where_in('nss_stud_id',$arr);
			$this->db->where('elig','Y');
			$query = $this->db->get();//echo $this->db->last_query();exit;
            return $query->row_array();
	}
	public function certi_new_ann($id)
	{
			...
			STUD.splcamp,STUD.tot_hr,STUD.elig,COL.college_name_for_gradecard, STUD.enroll_end, STUD.splcamp_start, STUD.splcamp_end, STUD.spl_desti,STUD.enrolled_date,STUD.iss_on,COL.college_district');
  		$this->db->where_in('STUD.nss_stud_id',$arr); 
  		$this->db->from('nss_stud STUD');
		$this->db->join('nss_college COL', 'STUD.college_id=COL.college_id',  'inner');
