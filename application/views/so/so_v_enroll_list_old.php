...
<?php if($enroll_list[0]['verification_id']=="3R"){ ?>
STATUS : REJECTED BY SO AND ITS FORWARDED TO RESPECTIVE PO ---- REMARKS:
<?php echo $enroll_list[0]['remarks'] ; }elseif($enroll_list[0]['verification_id']=="4"){ ?>
STATUS : ACCEPTED BY SO
<?php echo $enroll_list[0]['remarks'] ; } elseif($enroll_list[0]['verification_id']=="2R"){ ?>
STATUS : REJECTED BY ASSISTANT, FORWARDED TO SO ---- REMARKS:<?php echo $enroll_list[0]['remarks'] ; } }?>
</div>
 <?php if(isset($count_stud)&&!empty($count_stud)){?><span style="color:#0080FF; font-weight:bold;padding-left:40%"> TOTAL NUMBER OF STUDENT <?php echo $count_stud; ?> </span><?php }?>
	 <!-- table-->
...
<?php if(isset($enroll_list)&&!empty($enroll_list)&&($enroll_list[0]['verification_id']=="3"||$enroll_list[0]['verification_id']=="2R")){ ?>
<div class="w3-center">		
<input type="submit" class="btn btn-primary" value="ACCEPTED " name="acc" id="acc" style="background-color:#0080C0; color:#FFF"/>
