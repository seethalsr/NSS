...
<form name="frm1" id="frm1" method="post"   action="<?php echo base_url(); ?>Admin/NssSo/monthly_report" enctype="multipart/form-data"  >
...
<?php if(isset($get_yrs)){?>
        ...
        <td width="32%"><?php if(isset($year_sel)){?>
          <a href="<?php echo base_url(); ?>Po/NssPo/po_monthly_report_view/<?php echo $year_sel;?>" target="_blank" > View Yearly Report</a>
          <?php } ?></td>
        <td width="5%"></td>
		<td>&nbsp;</td>
		<td><?php if((isset($year_sel))&&(isset($month_sel))){?>
          <a href="<?php echo base_url(); ?>Po/NssPo/po_monthly_report_view/<?php echo $year_sel; echo $month_sel_n;?>" target="_blank"> View Monthly Report of <?php echo $month_sel; ?></a>
          <?php } ?></td>
        </tr>
		<?php } ?>
        </table>
		...
		<div class="w3-card w3-center" style="margin-top:20px;"><?php if(($monthly_report_data[0]['verification_id']=="4")){ ?>STATUS: ACCEPTED BY SO <?php }
 elseif(($monthly_report_data[0]['verification_id']=="3R")){ ?>STATUS: REJECTED BY SO  REASON:<?php echo $monthly_report_data[0]['remarks'] ;}?></div>
		<?php } ?>
		
		<?php if(isset($monthly_report_data[0]['verification_id'])&& $monthly_report_data[0]['verification_id']== "3" ) { ?>
		<input type="submit" value="ACCEPT" id="fwdtoassi" name="fwdtoassi"  />
